...
# Change 2/4/2014 --> Optmized the attenuator code, added  /r
# File creating bug is solved
#Bug: iperf server is created multiple times.
# Solved by using the killall command
import Tkinter
import tkFont
...
def reset_attn():
        s_att= serial.Serial(port='/dev/ttyACM0',baudrate=9600, bytesize=8, parity='N', stopbits=1,timeout=1) # Change according to the dmesg command  on linux
        s_w1= 'sa1 '+ str(0)+'\r'
        s_w2= 'sa2 '+ str(0)+'\r'
        s_w3= 'sa3 '+ str(0)+'\r'
        s_w4= 'sa4 '+ str(0)+'\r'
...
def Wifi_SSH_TCP(raw_f,attn,a5,a7,a9,a_window):
        ...
        cmd_rst= "killall iperf"                              #Iperf kill commmand
        print cmd_sr
        print cmd_si
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(hostname=a9, username='wireless',password='wireless',port=22)
        stdin, stdout, stderr = ssh.exec_command(cmd_rst) #kill all the iperf on the system
...
########Iperf Settings ######################################################
     ...
     ###########################################3 TCP/IP Window size###########################################################
        ...
        self.entry6.insert(0, "/home/wireless/Documents/Wireless_measurement_tool_TPVision/Measurement_1")  
...
if __name__ == "__main__":
    ...
    path= "C:\\Users\\mayur.sarode\\Documents\\EMC_testing\\new_EMC_tool\\Wireless_measurement_tool_TPVision\\Final_code\\TPvision-logo.tiff"
